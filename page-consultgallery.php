...
/* Template Name: Consultants Gallery */
...
<script language="javascript">	
	...
	function searchOffer()
	{
		...
		$.ajax({
			url:"<?php echo bloginfo('wpurl'); ?>/wp-admin/admin-ajax.php",
			type:'POST',
			data:'action=my_special_ajax_call&act=alo&tax=consultant-gallery&type=search&group_no=0&text='+ $('#offersearchinput').val(),
			success:function(results)
			{
				//alert(results);				
...
$total_groups = ceil($total_records/4);
?>
...
$(document).ready(function() {	
	...
	$.ajax({
		url:"<?php echo bloginfo('wpurl'); ?>/wp-admin/admin-ajax.php",
		type:'POST',
		data:'action=my_special_ajax_call&act=alo&tax=consultant-gallery&group_no=0' + path,
		success:function(results)
		{
			var params = results.split('][');
			$('.badge').html(params[0]);
		...
		//{
			if($('#track_load').val() <= $('#total_groups').val() && $('#loading').val()=='false') //there's more data to load
			{
				...
				$.ajax({
					url:"<?php echo bloginfo('wpurl'); ?>/wp-admin/admin-ajax.php",
					type:'POST',
					data:'action=my_special_ajax_call&act=alo&tax=consultant-gallery&group_no='+ $('#track_load').val() + path,
					success:function(results)
					{						
						var params = results.split('][');
						$('.badge').html(params[0]);
						...
						$('#total_groups').val(Math.ceil(params[0]/4));
...
<div id="videos_page">
  ...
  <?php
	...
	$offers = get_posts($args);	*/
	?>
  <div class="main_wrap" style="width:1120px; padding:0; margin:10px auto 0px;">
...
<h3 style="text-align:center; padding: 0px 0 40px;">Terms and Conditions Apply. <a onclick="showtospop('terms3pop');" href="javascript:void(0);"><strong>Please read</strong></a>.</h3>
...
<div class="dashboard"><?php
				echo do_shortcode('[videos id="2484" width="1000" height="457"]');
				?></div>
</div>
<div class="clear"></div>
</div>
